"""
Round 2 strategy (R2.1) — adapted from R1Algo.

Round 2 keeps the same two products (ASH_COATED_OSMIUM, INTARIAN_PEPPER_ROOT)
with the same position limits (80 each). The only new mechanic is the
Market Access Fee (MAF) blind auction — participants declare a bid via
Trader.bid(); the top 50% of bids get ~25% extra quote flow, and the
accepted bid is subtracted from the round's profit.

Trading logic:
  - INTARIAN_PEPPER_ROOT: aggressive trend rider (buy to max long, hold).
    Day -1 → Day 1 historical mids climb ~11,000 → ~14,000 (≈ +0.1/tick),
    which matches the trend observed in the R1 live submission.
  - ASH_COATED_OSMIUM: resin-style market making around a fair value of
    10,000 (historical mids oscillate within ±20).

MAF bid (see bid() at the bottom of the class): a one-time fee that is
only charged if we land in the top 50% of all submitted bids. Set to a
conservative integer; tune before the final submission based on how much
of the expected round PnL you want to spend for ~25% extra flow.

Limits & constraints (per IMC rules):
  - Trader class with run(state) -> (orders, conversions, traderData).
  - For R2 specifically, Trader also defines bid() returning an int.
  - Position limits: 80 long / 80 short per product, enforced by the exchange.
  - traderData is a string (JSON-serialized); kept well under 50,000 chars.
...
# --- Market Access Fee (R2 only) ---
# Top 50% of bids across all participants are accepted and pay their bid.
# Winners get ~25% extra quote flow for the whole round (5 days equivalent).
# Kept conservative: a few % of the expected round PnL buys a real edge
# without risking a large hit if the algo underperforms. Tune before submit.
MAF_BID = 2500
...
class Trader:
    ...
    def bid(self) -> int:
        # Blind-auction fee for extra 25% market access in Round 2.
        # Only deducted if we place in the top 50% of all bids.
        return MAF_BID
