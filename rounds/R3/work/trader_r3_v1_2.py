"""IMC Prosperity Round 3 — trader_r3_v1_2 (EMA fair, post-live diagnosis).

v1.1 worked on historical (+$112k across 3 days) but lost $3,209 on the live
submission because the OU center had drifted: HYDROGEL live mean = 9,979 vs
our pinned fair=10,000; VELVETFRUIT live mean = 5,262 vs our fair=5,250.

v1.2 replaces the constant fair with an EMA over the live mid, seeded from
the historical OU μ. The EMA tracks slow regime shifts; the seed prevents
ugly bootstrap-from-zero behavior on tick 0.

  1. HYDROGEL_PACK MM with EMA fair (α=0.02, half-life ~35 ticks).
  2. VELVETFRUIT_EXTRACT MM with EMA fair (α=0.02).
...
# EMA fair tracks slow drift in the OU center. α=0.02 → half-life ~35 ticks.
# Seed = Phase 1 OU μ estimates (used until the EMA has any data).
HYDRO_FAIR_SEED = 10_000.0
VELVET_FAIR_SEED = 5_250.0
FAIR_EMA_ALPHA = 0.02
...
class Trader:
    ...
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        try:
            ...
            data = {}
        if "hyd_fair" not in data:
            data["hyd_fair"] = HYDRO_FAIR_SEED
        if "vel_fair" not in data:
            data["vel_fair"] = VELVET_FAIR_SEED
        ...
        if HYDROGEL in state.order_depths:
            ...
            data["hyd_fair"] = update_ema(data["hyd_fair"], depth)
            pos = state.position.get(HYDROGEL, 0)
            ol: List[Order] = []
            mm_block(HYDROGEL, depth, pos, data["hyd_fair"],
        ...
        if VELVET in state.order_depths:
            ...
            data["vel_fair"] = update_ema(data["vel_fair"], depth)
            pos = state.position.get(VELVET, 0)
            ol: List[Order] = []
            mm_block(VELVET, depth, pos, data["vel_fair"],
