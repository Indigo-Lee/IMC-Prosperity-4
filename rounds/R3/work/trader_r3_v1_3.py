"""IMC Prosperity Round 3 — trader_r3_v1_3 (anchor + bounded running mean).

α-sweep on v1.2 showed any non-zero EMA hurt 4-day total: even α=0.0001 lost
$30k of historical PnL while saving only $200 on live. The MM block needs a
*stable* fair to capture mean reversion; chasing the mid kills the edge.

v1.3 keeps the historical anchor (10000 / 5250) as the long-run fair, but
allows it to slowly migrate by up to ±BOUND ticks if a sustained running
mean is observed. The migration uses a slow EMA (α=0.0005) but the *output*
is clipped, so on stable days the fair stays at the anchor.

  1. HYDROGEL_PACK MM, fair = clip(running_mean, anchor ± 30).
  2. VELVETFRUIT_EXTRACT MM, fair = clip(running_mean, anchor ± 15).
  3. Free-call lottery: BUY-ONLY VEV_6000/VEV_6500 at price 0.
...
# Anchor + bounded migration. Running mean drives fair, but clipped to stay
# within ±BOUND ticks of the historical anchor.
HYDRO_FAIR_ANCHOR = 10_000.0
VELVET_FAIR_ANCHOR = 5_250.0
HYDRO_FAIR_BOUND = 30.0   # live drifted ~21 ticks → 30 covers it with margin
VELVET_FAIR_BOUND = 15.0  # live drift was 12 ticks
FAIR_EMA_ALPHA = 0.0005   # slow EMA → half-life ~1400 ticks. The clip is the
                          # real safety; this α controls migration speed only.
...
class Trader:
    ...
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        try:
            ...
            data = {}
        if "hyd_ema" not in data:
            data["hyd_ema"] = HYDRO_FAIR_ANCHOR
        if "vel_ema" not in data:
            data["vel_ema"] = VELVET_FAIR_ANCHOR

        orders: Dict[Symbol, List[Order]] = {}

        def update_ema(prev: float, depth: OrderDepth) -> float:
            ...
            if ba <= bb:
                return prev
            mid = 0.5 * (bb + ba)
            return (1 - FAIR_EMA_ALPHA) * prev + FAIR_EMA_ALPHA * mid

        def clipped_fair(ema: float, anchor: float, bound: float) -> float:
            return max(anchor - bound, min(anchor + bound, ema))

        # ---- HYDROGEL_PACK MM ----
        if HYDROGEL in state.order_depths:
            depth = state.order_depths[HYDROGEL]
            data["hyd_ema"] = update_ema(data["hyd_ema"], depth)
            fair = clipped_fair(data["hyd_ema"], HYDRO_FAIR_ANCHOR, HYDRO_FAIR_BOUND)
            pos = state.position.get(HYDROGEL, 0)
            ol: List[Order] = []
            mm_block(HYDROGEL, depth, pos, fair,
        ...
        if VELVET in state.order_depths:
            ...
            data["vel_ema"] = update_ema(data["vel_ema"], depth)
            fair = clipped_fair(data["vel_ema"], VELVET_FAIR_ANCHOR, VELVET_FAIR_BOUND)
            pos = state.position.get(VELVET, 0)
            ol: List[Order] = []
            mm_block(VELVET, depth, pos, fair,
