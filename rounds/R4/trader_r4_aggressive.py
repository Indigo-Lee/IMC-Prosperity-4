...
COPY_PRODUCTS = (HG, VE)
CP_FORWARD_TICKS = 20
CP_MIN_PRODUCT_TRADES = 80    # was 20 — small-sample hit rates were noise
CP_HIT_RATE = 0.70            # was 0.68 — slightly tighter
CP_RECENT_WINDOW = 30
CP_RECENT_KILL_RATE = 0.58    # was 0.55 — must agree with full rate more strictly
CP_PENDING_CAP = 800
CP_MAX_SLIPPAGE = {
    HG: 8,
    VE: 4,
...
# Self-correcting kill switch: track copy-layer cash + mark-to-mid PnL per
# product. If the copy layer alone has lost more than CP_LOSS_LIMIT on a
# product, disable copy on that product for the rest of the run. This
# prevents any over-fitted counterparty rule from bleeding all day.
CP_LOSS_LIMIT = {
    HG: -700.0,
    VE: -500.0,
}
...
class Trader:
    def default_data(self) -> Dict[str, Any]:
        return {
            ...
            "cp": {
                ...
                "stats": {},
                "blocked": {},
                "acct": {},        # symbol -> {"cash": float, "pos": int}
                "killed": {},      # symbol -> {"timestamp", "pnl"}
            },
        }
    ...
    def cp_pnl(self, state: TradingState, data: Dict[str, Any], symbol: Symbol) -> float:
        """Mark-to-mid PnL of the copy layer alone for this product."""
        acct = data.get("cp", {}).get("acct", {}).get(symbol)
        if not acct:
            return 0.0
        mid = self.mid(state, symbol)
        if mid is None:
            return float(acct.get("cash", 0.0))
        return float(acct.get("cash", 0.0)) + float(acct.get("pos", 0)) * mid

    def cp_record_fill(self, data: Dict[str, Any], symbol: Symbol, price: int, qty: int) -> None:
        """qty positive for buy, negative for sell. Conservative: assumes order fills at sent price."""
        acct = data.setdefault("cp", {}).setdefault("acct", {}).setdefault(symbol, {"cash": 0.0, "pos": 0})
        acct["cash"] = float(acct.get("cash", 0.0)) - qty * float(price)
        acct["pos"] = int(acct.get("pos", 0)) + qty

    def copy_live_counterparties(self, state: TradingState, data: Dict[str, Any], orders: Dict[Symbol, List[Order]]) -> None:
        cp = data.setdefault("cp", {"pending": [], "stats": {}, "blocked": {}, "acct": {}, "killed": {}})
        killed = cp.setdefault("killed", {})

        for symbol in COPY_PRODUCTS:
            if symbol in data.get("disabled", {}):
                continue

            # Self-correcting copy-PnL kill switch (anti-overfit core mechanism)
            if symbol in killed:
                continue
            cp_running = self.cp_pnl(state, data, symbol)
            if cp_running < CP_LOSS_LIMIT[symbol]:
                killed[symbol] = {"timestamp": state.timestamp, "pnl": cp_running}
                logger.print("COPY_KILLED", symbol, f"pnl={cp_running:.1f}")
                continue
            ...
            if buy_signal > 0 and ask <= fair + CP_MAX_SLIPPAGE[symbol] and trend > -MAX_FAIR_GAP_TO_ADD[symbol]:
                ...
                if sent:
                    self.cp_record_fill(data, symbol, ask, sent)
            ...
            if sell_signal > 0 and bid >= fair - CP_MAX_SLIPPAGE[symbol] and trend < MAX_FAIR_GAP_TO_ADD[symbol]:
                ...
                if sent:
                    self.cp_record_fill(data, symbol, bid, sent)
...
"""
Plain-English summary (rev 3):
  Aggressive = conservative MM base (HG, VE, no vouchers) + live-only
  counterparty following with a self-correcting copy-PnL kill switch.

  - No Mark/historical IDs trusted. Each trader must score >= 70% over
    >= 80 evaluated forward moves AND keep their trailing-30 rate >= 0.58
    before we copy them. Copy layer trades only HG/VE.

  - The new anti-overfit core: cp_acct tracks the copy layer's own cash &
    position per product. If mark-to-mid copy PnL drops below CP_LOSS_LIMIT
    on a product, we permanently disable copy on that product for the run.
    This means: even if the hit-rate filter accepts a wrong trader, the
    layer caps its own bleeding at ~$500-$700 per product instead of
    chasing for the full day.

Key risks:
  The detector may activate late or never. The kill switch may fire on
  variance and disable a genuine signal — acceptable cost for the floor.

Most sensitive constants:
  CP_MIN_PRODUCT_TRADES (now 80), CP_HIT_RATE (0.70), CP_LOSS_LIMIT,
  CP_SUBLIMIT, PRODUCT_LOSS_LIMITS, RISK_LIMITS, anchor weights.
"""
